...
export default function Certificates() {
  ...
  return (
    <section id="certificates" className="w-full bg-gradient-to-b from-gray-950 to-green-950 pt-24 pb-48 scroll-mt-20">
      <div className="max-w-[1440px] mx-auto px-20">
        <ScrollReveal className="text-center mb-16">
          <h2 className="font-poppins text-green-300 text-4xl font-bold">Certificates</h2>
        </ScrollReveal>
        ...
        <div className="flex justify-center gap-8 flex-wrap">
          {certificates.map((cert, i) => (
            <ScrollReveal
              ...
              delay={i * 200}
              once={false}
              className="w-80 bg-green-950 border border-green-300 flex flex-col"
            >
              <img src={cert.img} alt={cert.name} className="w-full h-40 object-cover" />
              <div className="p-4 flex items-center justify-between">
                <p className="font-poppins text-green-300 text-lg">{cert.name}</p>
                <button
                  onClick={() => setSelected(cert)}
                  className="font-poppins text-green-300 text-base underline hover:text-green-100 transition-colors"
                >
                  lihat
                </button>
              </div>
